...
clear
figure(1); cla; hold on; grid on; view(3);
...
file_ind = 1:10; %[1 2 3 4 5];
% file_ind = [1 6 12 13];
% file_ind = [1 3 4 ];

%Loading data
for i=1:length(file_ind)
   x=load(sprintf('data_slave_arm/data_pos_%0.3d.txt',file_ind(i))); 
...
x=load(sprintf('data_slave_arm/data_pos_%0.3d.txt', ndem));         % Loading a demo for testing the coupling model. 
...
% ====== SAVING ======= 
structGMM = cplGMM; 
save('slave_cplGMM.mat','structGMM');
saveGMM('slave_cplGMM.mat','slave_cplGMM.txt');
...
[x0 , xT, Data, index] = preprocess_demos(pdata, ... % Takes the substracted position data. Computes first derivative and cuts traj
                        ...
                        0.25);                        % tolerance for cutting. If the derivative is below this value data is deleted
...
% ====== SAVING ======= 
structGMM = masterGMM;    
save('slave_posGMM.mat','structGMM');
saveGMM('slave_posGMM.mat','slave_posGMM.txt');
...
% ====== SAVING ======= 
structGMM = slaveGMM;
save('slave_oriGMM.mat','structGMM'); 
saveGMM('slave_oriGMM.mat','slave_oriGMM.txt');

%% Simulation using coupling
dt = 0.01;
load slave_cplGMM;    cplGMM = structGMM;     cplfnc = cplGMM.cplfunc;
load slave_posGMM; masterGMM = structGMM;
load slave_oriGMM;  slaveGMM = structGMM;
...
% For Simple (linear) slave dynamics. Replace this with CDDynamics or other
