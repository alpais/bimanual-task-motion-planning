...
protected:
    ...
    bool coupled_learned_model_execution(TaskPhase phase, CDSController::DynamicsType masterType, CDSController::DynamicsType slaveType,
                                 ...
                                 std::string r_model_base_path, std::string l_model_base_path, std::string force_gmm_id) {
	...
	char buf[1025];
	sprintf(buf, "%s/Phase%d/forceGMM_%s.txt", r_model_base_path.c_str(), phase, force_gmm_id.c_str());
	GMR* armcoupling_GMM = new GMR(buf);
	std::vector<int> in(1), out(1);
	in[0] = 0; out[0] = 1;
	armcoupling_GMM->initGMR(in, out);
	if(armcoupling_GMM->isInit) {
		return armcoupling_GMM;
	} else {
		delete armcoupling_GMM;
		return NULL;
	}
	...
	// >>> Initialize Stiffness
	    ...
	    // >>> Update coupling
	    //double crt_cpl_state = r_curr_ee_pose.getOrigin().norm();
	  //  double out_cpl_state = 0;
	  //  armcoupling_GMM->getGMROutput(crt_cpl_state, out_cpl_state));
